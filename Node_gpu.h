...
struct Node{
    ...
    __host__ __device__ virtual ~Node(){if(leftChild) free(leftChild); if(rightChild) free(rightChild);}
    Node* leftChild;
    Node* rightChild;
};

struct LeafNode: public Node{
    __host__ __device__ LeafNode():index(-1){}
...
struct InternalNode: public Node{
    __host__ __device__ InternalNode():start(-1), end(-1){}
